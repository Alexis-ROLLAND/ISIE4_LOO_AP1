...
#define TEST07_UNREGISTER_METHOD
...
TEST_CASE("testing the Bit class : Structure only") { std::println("Structure OK"); }
#endif
//----------------------------------------------------------------------
...
TEST_CASE("testing the Bit class : Ctors, getValue accessor and toString method") {
    ...
    CHECK(pBit->toString() == SET_STR);
    delete pBit;

    pBit = new Bit(Bit::bit_t::CLEAR);
    CHECK(pBit->toString() == CLEAR_STR);
    delete pBit;

    pBit = new Bit(Bit::bit_t::HIZ);
    CHECK(pBit->toString() == HIZ_STR);
    delete pBit;

    pBit = new Bit(Bit::bit_t::X);
    CHECK(pBit->toString() == X_STR);
    delete pBit;
#endif
}
#endif
...
#ifdef TEST07_UNREGISTER_METHOD
    myBit.unregister();
    CHECK(myBit.getValue() == Bit::bit_t::X);
#endif
}
